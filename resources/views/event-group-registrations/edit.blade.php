<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Редактировать регистрацию группы
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('event-group-registrations.update', $eventGroupRegistration) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            ...
                            <select name="events_eventID" id="events_eventID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach ($events as $event)
                                    <option value="{{ $event->eventID }}" {{ old('events_eventID', $eventGroupRegistration->events_eventID) == $event->eventID ? 'selected' : '' }}>{{ $event->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="groups_groupID" id="groups_groupID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                @foreach ($groups as $group)
                                    <option value="{{ $group->groupID }}" {{ old('groups_groupID', $eventGroupRegistration->groups_groupID) == $group->groupID ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="statusGroupRegistration" id="statusGroupRegistration" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                <option value="active" {{ old('statusGroupRegistration', $eventGroupRegistration->statusGroupRegistration) == 'active' ? 'selected' : '' }}>Активно</option>
                                <option value="cancelled" {{ old('statusGroupRegistration', $eventGroupRegistration->statusGroupRegistration) == 'cancelled' ? 'selected' : '' }}>Отменено</option>
                            </select>
                        </div>
