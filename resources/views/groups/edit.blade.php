                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('groups.update', $group) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="groupName" id="groupName" value="{{ $group->groupName }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="specialties_specialityID" id="specialties_specialityID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($specialties as $specialty)
                                    <option value="{{ $specialty->specialityID }}" @if($group->specialties_specialityID == $specialty->specialityID) selected @endif>{{ $specialty->specialityName }}</option>
                                @endforeach
                            </select>
                        </div>
