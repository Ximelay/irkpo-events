                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('specialties.update', $specialty) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="specialityName" id="specialityName" value="{{ $specialty->specialityName }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>
                        <div class="mb-4">
                            <label for="specialityCode" class="block text-gray-700 text-sm font-bold mb-2">Код специальности:</label>
                            <input type="text" name="specialityCode" id="specialityCode" value="{{ $specialty->specialityCode }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="faculties_facultyID" id="faculties_facultyID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($faculties as $faculty)
                                    <option value="{{ $faculty->facultyID }}" @if($specialty->faculties_facultyID == $faculty->facultyID) selected @endif>{{ $faculty->facultyName }}</option>
                                @endforeach
                            </select>
                        </div>
