        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('event-registrations.update', $eventRegistration) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            ...
                            <select name="events_eventID" id="events_eventID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($events as $event)
                                    <option value="{{ $event->eventID }}" @if($eventRegistration->events_eventID == $event->eventID) selected @endif>{{ $event->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="users_userID" id="users_userID" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->userID }}" @if($eventRegistration->users_userID == $user->userID) selected @endif>{{ $user->user_lastName }} {{ $user->user_firstName }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            <label for="statusEventRegistration" class="block text-gray-700 text-sm font-bold mb-2">Статус:</label>
                            <input type="text" name="statusEventRegistration" id="statusEventRegistration" value="{{ $eventRegistration->statusEventRegistration }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        </div>
